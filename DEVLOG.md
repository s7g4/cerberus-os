...
## Milestone 22 — Full Engineering Audit: Secure Boot Honesty, Host Test Coverage, Doc/Code Consistency
- **Goal**: A full red-team-then-fix pass over the whole repo, not just the concurrency path: is every claim in the docs actually true of the code, does anything host-testable have tests, and are there any remaining `unwrap()`/PMP-match/logic gaps left over from earlier milestones.
- **What Broke & How it Was Fixed**:
  - *Issue 1 — Secure boot was tautological*: `verify_tampered_secure_boot` compared two unrelated hardcoded strings instead of mutating a real copy of the trusted payload, so "tamper detection" never exercised the actual hash-mismatch path it claimed to demonstrate.
    - *Fix*: Rewrote `bootloader.rs` to flip a byte in a real copy of `TRUSTED_PAYLOAD` before re-hashing, added honest module docs stating the checksum is not real ECDSA (an earlier `p256`-based revision blew the `.text` budget, see Milestone 19), and added 4 real unit tests.
  - *Issue 2 — Host-testable crates had zero tests, and getting there exposed two real cross-compilation bugs*: `security`, `network`, `scheduler`, and `telemetry` are all plain Rust logic underneath their `no_std` gate, but none had a single test. Making `security`'s new tests actually run (`cargo test --target x86_64-unknown-linux-gnu`) failed with 18 "invalid register" errors from `telemetry::syscalls`' RISC-V-specific `core::arch::asm!` blocks, which were gated only behind `#[cfg(not(kani))]` — true on a host build too.
    - *Fix*: Extended the gate to `#[cfg(all(target_arch = "riscv32", not(kani)))]` / `#[cfg(any(kani, not(target_arch = "riscv32")))]`, mirroring the existing kani-dummy-body pattern. Added `#![cfg_attr(not(test), no_std)]` to `security`, `network`, `scheduler`, `telemetry` and wrote 20 tests total (CAN frame parsing + ring buffer, scheduler partition-swap behavior, TCB stack-frame initialization, secure-boot tamper detection, Postcard round-trips).
  - *Issue 3 — `TaskControlBlock::initialize_stack` had a latent 32/64-bit portability bug, caught by its own new test*: the function reserved a hardcoded `128` bytes (32 registers × 4-byte words) for the trap frame regardless of `size_of::<usize>()`. On the real RV32 target this is correct; compiled for the x86_64 test host (8-byte `usize`) it silently overflowed the stack array by 128 bytes and segfaulted the test binary.
    - *Fix*: Reserve `32 * core::mem::size_of::<usize>()` bytes instead of a hardcoded `128` — identical value (128) on the real target, but now portable.
  - *Issue 4 — `telemetry`'s own `get_hart_id`/`log_telemetry` had the same unguarded-asm bug as Issue 2, masked by dead-code elimination*: fixing Issue 2 only touched `syscalls.rs`; `lib.rs`'s own `csrr mhartid` read and its reference to the bare-metal-only `SEGGER_RTT_WriteNoLock` extern symbol were still gated on `#[cfg(not(kani))]` alone. Host tests passed anyway at the time, purely because `[profile.dev]` mirrored `[profile.release]` (`lto = true`, `opt-level = "z"`) and LTO eliminated the dead, invalid code before the assembler ever validated the `csrr` mnemonic against the x86_64 backend.
    - *Fix*: Applied the identical `target_arch`-gated fix to `lib.rs`. Separately, relaxed `[profile.dev]` to drop `lto`/`opt-level`/`codegen-units` (kept `panic = "abort"`, a hard `no_std` requirement, not an optimization) for faster local iteration — which is what surfaced Issue 4 as a hard compile error instead of a silently-passing accident in the first place.
  - *Issue 5 — `reprogram_pmp_stack`'s wildcard match left the HSM stack unprotected whenever an idle task was active*: the function only ever tracked 4 dynamic PMP slots for 5 real task stacks (Watchdog, A, B, C, HSM); a `_ =>` arm handled "HSM active" and both idle tasks identically, which happened to be correct for HSM (blocks the other 4) but wrong for idle (leaves HSM's stack reachable, since idle isn't running as HSM and has no reason to leave it open).
    - *Fix*: Added a 5th dynamic PMP entry (6) — `configure_pmp` only supported entries 0–5 before this — and rewrote the function as a loop over all 5 (name, stack, entry) tuples: the entry matching the active task's name is set to `Off` (falls through to the broad SRAM allow rule), every other entry blocks its stack. A name matching none of the five (both idle tasks, or anything unrecognized) now correctly blocks all 5.
  - *Issue 6 — Two flagship docs contradicted each other, and one described a data structure that never existed*: `README.md` presented "Priority Inheritance Protocol" as a live feature, including a specific `priority_to_task: [Option<u8>; 32]` array; `ARCHITECTURE.md` correctly said PIP was removed as obsolete once ARINC-653 time partitioning replaced priority scheduling (ADR-014). Grepping the codebase confirmed `priority_to_task` never existed and `active_priority` is never mutated after registration.
    - *Fix*: Rewrote README's section to match reality: PIP shipped (ADR-011), was superseded (ADR-014), and what remains is highest-*base*-priority-waiter wakeup via each mutex's `waiters_bitmap` — no active-priority boosting. Also corrected the same section's PMP description (it still said "Entries 1, 2, and 4" / "three inactive tasks", pre-dating Issue 5's fix).
  - *Issue 7 — README/RESEARCH claimed a `flip-link` stack-overflow guard page that was never wired into the build*: `flip-link` appears nowhere in `Cargo.lock`, `Cargo.toml`, or `.cargo/config.toml`'s linker args — only stock `riscv-rt` linker scripts are used. The gap this was supposed to cover is real: PMP only blocks the 4 *inactive* stacks per switch; nothing guards the active task's own stack from overflowing into an adjacent static.
    - *Fix*: Corrected README and RESEARCH.md to state the true protection scope, and documented the gap in `SECURITY.md`'s known-limitations list rather than silently dropping the claim.
  - *Issue 8 — Minor hygiene*: `HMAC_TAG_LEN` was independently defined in both `hmac.rs` and `hsm.rs` (same value today, no shared source of truth — a latent risk if one is edited without the other, since `sys_recv` truncates on a length mismatch rather than erroring); the HSM's hardcoded dev key lacked the same honest-limitation documentation already given to the secure-boot checksum; `telemetry_broker.py` bound to `0.0.0.0` with no reason to (the dashboard only ever connects via `127.0.0.1`).
    - *Fix*: `hsm.rs` now imports `HMAC_TAG_LEN` from `hmac.rs` instead of redeclaring it; added a module-doc caveat on `HMAC_KEY` plus a `SECURITY.md` bullet; changed the broker's bind address to `127.0.0.1`.
- **Time Log**:
  - Secure boot rewrite + tests: 45m
  - Extending host-test coverage across 4 crates, including diagnosing and fixing the two cross-compilation bugs it exposed (Issues 2–4): 2h
  - `unwrap()`/`expect()` invariant audit across `trap.rs` (33 call sites, all traced to sound scheduler invariants; documented rather than "fixed" since converting them to defensive branches would be dead code that can never execute): 40m
  - PMP wildcard fix, including extending `configure_pmp` to a 5th entry: 45m
  - Doc-vs-code consistency pass (Issues 6–7) — the PIP and flip-link findings were caught by grepping for the specific implementation details each doc claimed and confirming they don't exist in the code, not by reading the docs alone: 1h
  - `CONTRIBUTING.md`, `SECURITY.md`, `dependabot.yml`, dev/release profile split: 40m
- **Metric Captured**:
  - Clean release build: `.text` = 25,884 bytes (budget 32,768), `.bss` = 10,304 bytes, zero heap allocations, zero FPU instructions.
  - 20 host-side unit tests passing across `security` (4), `scheduler` (8), `network` (5), `telemetry` (3) — none existed before this milestone.
  - `cargo fmt --check` and `cargo clippy --target riscv32imac-unknown-none-elf -- -D warnings` both clean on the bare-metal target, in both `dev` and `release` profiles.
